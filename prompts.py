...
def response_template(url, page_desc, chat_history):
    prompt = f"""
...
def page_summary_template(web_content):
    prompt = f"""Summarize the following website given the following links, buttons, and text inputs:
        {web_content}
...
def objective_template(chat_history):
    prompt = f"""Given the following chat history, determine the user's objective based on the recent chat history:
        {chat_history}
...
def command_template(browser_content, page_description, url, previous_command, objective):
    prompt = f"""
